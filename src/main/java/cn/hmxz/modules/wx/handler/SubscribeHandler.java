...
import cn.hmxz.modules.wx.service.MsgReplyService;
...
public class SubscribeHandler extends AbstractHandler {
    ...
    public WxMpXmlOutMessage handle(WxMpXmlMessage wxMessage, Map<String, Object> context, WxMpService wxMpService,
        ...
        if (StringUtils.hasText(wxMessage.getEventKey())) {
            // 记录推广数据
            String key = wxMessage.getEventKey().substring(8);
            if (StringUtils.hasText(key)) {
                WxQrUser wxQrUser = new WxQrUser();
                wxQrUser.setOpenId(wxMessage.getFromUser());
                wxQrUser.setQrKey(key);
                wxQrUserMapper.insert(wxQrUser);
                wxQrCodeMapper.updateNumber(key);
            }
            // 处理特殊事件，如用户扫描带参二维码关注
